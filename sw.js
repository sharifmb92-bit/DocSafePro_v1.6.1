/* =========================================================
   DocSafe 1.6.1 - Service Worker & Motor PWA
   Sello: medbasha
   ========================================================= */

const CACHE_NAME = 'docsafe-v1.6.1';
const URLS_TO_CACHE = [
  './',
  './index.html',
  './sw.js',
...
// 3. CAPTURA DE COMPARTIR DESDE WHATSAPP / CORREO (EVITA ERROR 405)
self.addEventListener('fetch', (event) => {
  const url = new URL(event.request.url);

  // Intercepta peticiones POST dirigidas a /share-target
  if (event.request.method === 'POST' && url.pathname.includes('share-target')) {
    event.respondWith(
      (async () => {
        ...
        // Redirige limpiamente a la app cliente evitando el servidor
